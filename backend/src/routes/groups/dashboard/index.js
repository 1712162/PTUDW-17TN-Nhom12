  ...
  Group.findById(req.params.id).populate("comments").exec(function (err, foundGroup) {
    if (err) {
      ...
      if (newGroup.status === true)
        ...
        res.render('groups/group-details-member-view', { group: [] });
    }
  })
});
  ...
  User.findById(req.user._id)
    ...
    .exec(function (err, user) {
      if (err) {
        ...
        res.render('groups/dashboard', { 'groups': dashboardGroup });
      }
    });
});
  ...
  User.findById(req.user._id)
    .exec((err, foundUser) => {
      if (err) {
        ...
        foundUser.groups.myjoingroup = foundUser.groups.myjoingroup.filter((group) => {

      ...
      if (err) {
        ...
        User.findById(req.user._id).exec(function (err, user) {
          if (err) {
            ...
            else res.render("groups/group-details-user-view", { group: [] });
          }
        });
      }
    });
    ...
    if (err) {
      ...
      groups.forEach((group) => {
       ...
       newGroup.owners.forEach((ownerId) => {
        ...
        if (check === false && group.status === true) newGroups.push(newGroup);
      });
...
router.post("/explore/enroll", function (req, res) {
  Group.findById(req.body.group_id_enroll).exec((err, group) => {
    if (err) {
      ...
      if (check === true) group.waiting_list.push({ userenroll: req.user._id });
      group.save();
      ...
      User.findById(req.user._id).exec((err, user) => {
        if (err) {
          ...
          user.groups.myenrollrequestgroup.forEach((group) => {
            if (
              group.group.toString() === req.body.group_id_enroll.toString()
            ) {
              check = false;
            }
          });
          if (check === true) {
            user.groups.myenrollrequestgroup.push({
              group: req.body.group_id_enroll,
            });
            user.save();
          }
